...
fn main() -> Result<(), StegError> {
    ...
    match args.len() {
        3 => {
            ...
            let paths = fs::read_dir(&Path::new(&args[2])).unwrap();
            let names = paths
                .map(|entry| {
            ...
            let pool=ThreadPool::new(count.clone());
        ...
        5 => {
            let mut valid_files: Vec<String> = Vec::new();
            let paths = fs::read_dir(&Path::new(&args[3])).unwrap();
            match fs::create_dir(&args[4]) {
                    ...
                    for k in 0..*count {
                        ...
                        if (index <= valid_files.len() - 1) {
                          //  println!("Inside Index:{}", index);
                            ...
                            let out_directory_clone = args[4].clone();
...
{
  //  println!("Coming inside encode_message");
